...
class Model(nn.Module):
    def __init__(self, action_dims, dueling):
        super().__init__()
        if Config.model_version == 0:
            ...
            self.fc1 = nn.Sequential(
                nn.Linear(in_features=5, out_channels=256),
                nn.ReLU())

            self.fc2 = nn.Sequential(
                nn.Linear(in_features=512, out_channels=128),
                nn.ReLU())

            self.multi_output = nn.ModuleList([nn.Linear(128, dim) for dim in action_dims]) 
    ...
    def forward(self, observation):
        ...
        if Config.model_version == 0:
            lstm1_out = self.lstm1(observation[])         # output shape(batch, seq, input_size) as batch_first is used, (batch, 15, 5)
            fc1_out = self.fc1(observation[])             # output shape(batch, hidden_size) as batch_first is used, (batch, 256)   
